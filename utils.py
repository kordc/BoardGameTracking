...
def plot_circles(frame, circles):
    # plot found circles on the frame
    for i in circles[0, :]:
...
def centres_within(boxes1, boxes2, sigma_center=10, sigma_area=100):
    ...
    # & (area_diff < sigma_area))
    similar = np.argwhere((center_diff < sigma_center))
    return similar


def get_boxes(boxes):
    # This function takes a matrix of boxes and returns 4 vectors of coordinates
...
def iou(boxes2, boxes1):
    ...
    # from num_of_boxes by 4 matrix we go to 4 vectors.
    x21, y21, x22, y22 = get_boxes(boxes2)
    # transpose is done to make it broadcastable and has this maximum over all boxes!
...
def segment_colors(frame, debug=False):
    # segment colors for counters, ships and warriors
    yellow = segment_by_hsv_color(frame, np.array(
        [20, 100, 100]), np.array([30, 255, 255]))
    black = segment_by_hsv_color(frame, np.array(
        [0, 0, 0]), np.array([180, 255, 30]))
    red = segment_by_hsv_color(frame, np.array(
        [0, 100, 100]), np.array([10, 255, 255]))

    if debug:
        cv2.imshow("segmented_colors", np.concatenate(
            [yellow, black, red], axis=1))
        cv2.waitKey(0)
    return yellow, black, red


def segment_colors_cities(frame):
    # segment colors for cities
    blue = segment_by_hsv_color(frame, np.array(
        [90, 50, 50]), np.array([110, 255, 255]))
    purple = segment_by_hsv_color(frame, np.array(
        [130, 50, 50]), np.array([150, 255, 255]))
    white = segment_by_hsv_color(frame, np.array(
        [0, 0, 200]), np.array([180, 30, 255]))
    orange = segment_by_hsv_color(frame, np.array(
        [10, 100, 100]), np.array([20, 255, 255]))
    return blue, purple, white, orange


def cut_obj(frame, box):
    # cut object from the frame
    x, y, w, h = box
    return frame[y:y+h, x:x+w]


def get_land_mask(frame, label_circles=False):
    # segment land from the frame
    if label_circles:
...
def update_interesting_objects(foreground, frame, candidates, being_seen_limit, left=False, debug_contours=False):
    ...
    for cnt in cnts:
        ...
        if w*h > 80 and w*h < 10000:
            # Getting coordinates of every new found box
    ...
    if not candidates:  # if this is first iteration
        ...
        # which old box match to which new box
        matches = centres_within(np.array(old), np.array(new))
...
def get_font_color(color):
    # get font color for the text from the color of the object
    if color == "yellow":
